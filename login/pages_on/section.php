 



<div class="div_group">

    <div class="div_group_n1" id="div_group_n1">
            <div class="personnel"> Personnel </div>
 <!--
            <div class="display_flex2">
                <div>div_group_n1</div>
                <div class="n_nombre">154</div>
            </div>
-->

    </div>
    <div class="div_group_n2" id="div_group_n2">         
        
    <div class="personnel"> Liste projet </div>
    <div>
     

    <div class="div_group_n3" id="div_group_n3">
          <!--  <div class="carte">
                <div class="carte_0">
                    01
                </div>
            </div>
-->
 
 

    </div>



    </div>
 
 


</div>

</div>


<style>
    .div_group{
        border:1px solid rgba(0,0,0,0.1) ; 

        max-height:400px;


     
 
  overflow-y: scroll;
  scrollbar-color: rebeccapurple green;
  scrollbar-width: thin;
  display:flex;
  justify-content:space-around ; 
 margin-top:50px; 
 flex-wrap: wrap;

    }

    .div_group_n1 div {
 
margin:0 ; 
padding:15px;
 

    }

    .div_group_n1 div:hover {
 
 

    }

    .div_group_n1{
        width:50% ; 
    }
    .display_flex2{
        display:flex ; 
        justify-content:space-around ; 

    }
    .div_group_n1{
        
        text-align:center ; 
    }
    .display_flex2  {
        background-color:rgba(0,0,0,0.9) ; 
        width:100%;
        color:white; 
    }
 
    .display_flex2:hover{
        background-color:rgba(0,50,200,0.9) ; 
        cursor:pointer ; 
    }
    .n_nombre{
        background-color:rgba(200,50,200,0.7) ; 
        width:150px;
        padding:15px;        
    }
    .div_group_n2{
        width:40%;
    }
    .carte{
        width:150px; 
        height:150px;
        border:1px solid rgba(0,0,0,0.3) ;
 background-image:url("data:image/jpeg;base64,") ; 

    }
    .div_group_n3{
        display:flex;
        justify-content:space-around ; 
        flex-wrap: wrap;
        width:95% ;
        margin:auto ;  
 

        
    }
    .div_group_n3 div {
       
        margin-top:100px; 

    }
    .carte_0{
        background-color:rgba(200,50,200,0.7) ; 
        color:white ; 

        text-align:center ; 
    }
    .carte{
        border-radius:15px;
    }
    .carte:hover{
       cursor:pointer ; 

    }
    .personnel{
        background-color:black;
        color:white ; 
        padding:15px;
        text-align:center ; 
    }
</style>


<script>
    const myTimeout = setTimeout(myGreeting, 500);

function myGreeting() {
   


 



 
 
var node = document.createElement("div");
node.setAttribute("id","elements") ;  
node.setAttribute("class","display_flex2") ;  
document.getElementById("div_group_n1").appendChild(node);


var node = document.createElement("div");
var textnode = document.createTextNode("Element sans groupe");


node.appendChild(textnode);
document.getElementById("elements").appendChild(node);


var node = document.createElement("div");
var textnode = document.createTextNode("150");
node.setAttribute("class","n_nombre") ;  



node.appendChild(textnode);
document.getElementById("elements").appendChild(node);
 

// ELEMENT N°2 



 



 



 
var node = document.createElement("div");
node.setAttribute("id","elementsy") ;
node.setAttribute("class","carte") ;  

document.getElementById("div_group_n3").appendChild(node);




var node = document.createElement("div"); 
node.setAttribute("class","carte_0") ; 
node.innerHTML="ok" ; 

document.getElementById("elementsy").appendChild(node);


 



}



</script>